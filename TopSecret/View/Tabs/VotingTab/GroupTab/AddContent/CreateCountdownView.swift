    ...
    var body: some View {
        ZStack{
            ...
            VStack{
                ...
                Button(action:{
                    groupVM.createCountdown(group: group, countdownName: countdownName, startDate: Timestamp(), endDate: countdownDate, user: userVM.user ?? User())
                    presentationMode.wrappedValue.dismiss()
                },label:{
                    Text("Create Countdown").foregroundColor(Color("Foreground"))
                        .padding(.vertical)
