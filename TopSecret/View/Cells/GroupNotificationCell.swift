...
struct GroupNotificationCell: View {
    ...
    var body: some View {
        VStack{
            ...
            case "eventCreated":
                EventCreatedNotificationCell(groupNotification: groupNotification)
    ...
    var body: some View {
            ...
            VStack{
                HStack{
                    ...
                    WebImage(url: URL(string: groupNotification.notificationCreator?.profilePicture ?? ""))
                        .resizable()
                        .scaledToFill()
                        .frame(width:40,height:40)
                        .clipShape(Circle())
                    
                    Text("\(groupNotification.notificationCreator?.username ?? "USER_USERNAME") has created an event!").fontWeight(.bold).foregroundColor(FOREGROUNDCOLOR)
                    Spacer()
        ...
        VStack{
            HStack{
                WebImage(url: URL(string: groupNotification.notificationCreator?.profilePicture ?? ""))
                    ...
                    .frame(width:40,height:40)
                    .clipShape(Circle())
                Text("\(groupNotification.notificationCreator?.username ?? "USER_USERNAME") has created a countdown!").fontWeight(.bold).foregroundColor(FOREGROUNDCOLOR)
                Spacer()
                Text("\(groupNotification.notificationTime?.dateValue() ?? Date(), style: .time)").foregroundColor(.gray).font(.footnote)
            }
        }.padding().background(RoundedRectangle(cornerRadius: 16).fill(Color("Color"))).padding(10)
